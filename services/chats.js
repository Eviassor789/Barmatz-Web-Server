...
import { getChatsListOfUserByUsername, getProfilePicOfUserByUsername, getDisplasyNameUserByUsername, readUserByName} from "../services/users.js";
...
const createChat = async (user1, user2) => {
    ...
    return await chat.save().then(savedchat => {
        console.log('User saved with ID:', savedchat.id);
      })
      .catch(error => {
...
const addChat = async(username, friendUserName) => {
    ...
    response_json.user = await getUserDetailsByUsername(friendUserName); //SHOULD IT BE friendUserName ??? ////////////////////////////////////////////
